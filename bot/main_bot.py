...
def format_requisites(raw):
    ...
    # Единая нормализация (core.requisites) — общая для бота, /pay и Mini App:
    # ссылка в поле номера → payment_link, плейсхолдеры и дубли убираются,
...
async def menu_sell(callback: CallbackQuery, state: FSMContext):
    ...
    # Ставка выкупа берётся из core.pricing, а не вписывается сюда цифрой: текст
    # «19–27%» пережил бы смену ставки молча и обещал бы курс, которого больше
    # нет. Строка исчезает целиком, если ставку не прочитать — иначе клиент
    # увидел бы «рыночный минус  — ставка не зависит от суммы» (нашёл codex).
    _fee = sell_commission_label()
    _fee_line = (f"💱 Курс: рыночный минус <b>{_fee}</b> — ставка не зависит "
                 f"от суммы ({coins_line()})" if _fee else "")
    await callback.message.answer(
