...
- 2026-09-09: TONCONNECT_SDK_HANDOFF_WAIT_RECOVERY VERIFIED/deployed (`a30a806`).
  ...
  Prior verification/intent/preparation/Buy/Sell slices remain verified via receipts.
  Autopilot failed/MainPID0; reconcile stale supervisor receipts before explicit start.
...
- 2026-08-26 commit `221f09b` makes the established Wallet address book usable
  as a receive/send reference: users can copy the full address and name an
  existing address before reusing it for an exchange. Naming calls only the
  existing owner-scoped address-note preference route; it cannot add an
  address, access a key, request a signature, invoke a provider or create a
  payment. Mini App syntax, focused E4 tests 29/29 and standalone address-book
...
- 2026-08-26 commit `3fe3c01` turns existing Activity facts into a clear next
  step: receipt-under-review, delayed paid, expired, failed and cancelled cards
  explain their state, and the relevant cards explicitly say not to pay again.
  Existing payment, transaction-evidence and support links are unchanged. Mini
  App syntax, focused E4 tests 30/30 and standalone address-book checks 63/63
  pass; production `/webapp` returned `200`, bot/Relay are active. Rollback
  preimage:
