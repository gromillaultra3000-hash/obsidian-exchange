...
ROOT = Path('/root/output/manual/e4-payment-requisites-copy-20260908')
DOCS = Path('/root/docs/e4-payment-requisites-copy')
SOURCE = Path('/root/relay/webapp.html')
TARGET = Path('/opt/obsidian-exchange/relay/webapp.html')
OLD = '6bdad471edcedf7b21d28373cdd564268d50f62b16385bac9905215ad4df2db8'
NEW = 'f10c4ca6bbe8935822433c300d4286878e1c460255b780ae33dcba16a9e532c8'
...
def replace(target, expected, content, metadata):
    ...
    fd, name = tempfile.mkstemp(prefix='.payment-requisites-copy-', dir=target.parent)
    try:
...
def main(mode):
    ...
    if mode == 'rehearse':
        ...
        for name in ('acceptance-review.json', 'security-review.json'):
            ...
            assert review['inputsSha256']['tests/e4_review_browser.cjs'] == sha(Path('/root/tests/e4_review_browser.cjs').read_bytes())
        ...
        public_check(old)
        metadata = TARGET.stat()
        backup = Path(tempfile.mkdtemp(prefix='e4-payment-requisites-copy-20260908-',
