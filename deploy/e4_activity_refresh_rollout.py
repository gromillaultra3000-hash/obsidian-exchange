...
ROOT = Path('/root/output/manual/e4-activity-refresh-20260908')
DOCS = Path('/root/docs/e4-activity-refresh')
SOURCE = Path('/root/relay/webapp.html')
TARGET = Path('/opt/obsidian-exchange/relay/webapp.html')
OLD = 'e06b85edd00f0a877b8e75b848713e46c0f800ff37809e5cd085bcac63a4f983'
NEW = '6842d35ccb8d7bbbf2cac709cbc02c64d47bff527a5acda9699c4897d8b1da05'
...
def replace(target, expected, content, metadata):
    ...
    fd, name = tempfile.mkstemp(prefix='.activity-refresh-', dir=target.parent)
    try:
...
def main(mode):
    ...
    if mode == 'rehearse':
        ...
        public_check(old)
        metadata = TARGET.stat()
        backup = Path(tempfile.mkdtemp(prefix='e4-activity-refresh-20260908-',
