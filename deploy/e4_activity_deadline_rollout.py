...
ROOT = Path('/root/output/manual/e4-activity-deadline-20260908')
DOCS = Path('/root/docs/e4-activity-deadline')
SOURCE = Path('/root/relay/webapp.html')
TARGET = Path('/opt/obsidian-exchange/relay/webapp.html')
OLD = '6842d35ccb8d7bbbf2cac709cbc02c64d47bff527a5acda9699c4897d8b1da05'
NEW = 'efcc97b6e41f8972b39163a876196f45fd8c5562dad23b5769634aac8ba7b499'
...
def replace(target, expected, content, metadata):
    ...
    fd, name = tempfile.mkstemp(prefix='.activity-deadline-', dir=target.parent)
    try:
...
def main(mode):
    ...
    if mode == 'rehearse':
        ...
        public_check(old)
        metadata = TARGET.stat()
        backup = Path(tempfile.mkdtemp(prefix='e4-activity-deadline-20260908-',
