...
ROOT = Path('/root/output/manual/e4-order-support-20260908')
DOCS = Path('/root/docs/e4-order-support')
SOURCE = Path('/root/relay/webapp.html')
TARGET = Path('/opt/obsidian-exchange/relay/webapp.html')
OLD = 'f10c4ca6bbe8935822433c300d4286878e1c460255b780ae33dcba16a9e532c8'
NEW = 'e06b85edd00f0a877b8e75b848713e46c0f800ff37809e5cd085bcac63a4f983'
...
def replace(target, expected, content, metadata):
    ...
    fd, name = tempfile.mkstemp(prefix='.order-support-', dir=target.parent)
    try:
...
def main(mode):
    ...
    if mode == 'rehearse':
        ...
        public_check(old)
        metadata = TARGET.stat()
        backup = Path(tempfile.mkdtemp(prefix='e4-order-support-20260908-',
