"""Hash-bound E4 payment-expiry rendering deployment; only the Relay service may be restarted.
...
REPO = Path('/root')
ROOT = REPO / 'output/manual/e4-payment-expiry-20260908'
DOCS = REPO / 'docs/e4-payment-expiry'
LIVE = Path('/opt/obsidian-exchange')
PREIMAGES = Path('/var/lib/obsidian-exchange/deployment-preimages')
RECIPE = 'deploy/e4_payment_expiry_rollout.py'
MAIN = 'relay-fastapi/main.py'
HTML = 'relay/webapp.html'
FILES = (MAIN,)
BASELINE = {item['path']: item['sha256'] for item in [
    {'path': MAIN, 'sha256': '4ee1c62ee72734f0f403ec1a438f829fb800989f893dca2b24a184eb5cd1ecf7'},
]}
...
def write_report(path, data):
    fd, name = tempfile.mkstemp(prefix='.payment-expiry-report-', dir=path.parent)
    try:
...
def replace(target, expected, content, saved):
    ...
    fd, name = tempfile.mkstemp(prefix='.payment-expiry-', dir=target.parent)
    try:
        with os.fdopen(fd, 'wb') as stream:
...
def presentation_scope():
    ...
    def normalized(data):
        require(data.count(script_start) == 1, 'presentation_scope_markers')
        first = data.index(script_start)
        last = data.index(script_end, first)
        return data[:first] + b'<script>\n// presentation\n' + data[last:]

    require(normalized(old) == normalized(new), 'nonpresentation_source_changed')

    def interpolation_nodes(data):
        return [ast.dump(node, include_attributes=False) for node in ast.walk(ast.parse(data))
                if isinstance(node, ast.FormattedValue)]

    require(interpolation_nodes(old) == interpolation_nodes(new), 'python_interpolation_changed')
    return {'outsideInlineScriptByteExact': True,
            'allPythonInterpolationsExact': True,
            'numericPaymentPageUnchanged': True,
...
def gates(candidate):
    for name in ('tests.json', 'acceptance-review.json', 'security-review.json',
        ...
        if name == 'payment-browser-report.json':
            ...
            require(report['runnerSha256'] == file_state(REPO / 'tests/e4_payment_expiry_browser.cjs'),
...
def deploy():
    ...
    backup = Path(tempfile.mkdtemp(prefix='e4-payment-expiry-20260908-', dir=PREIMAGES))
    backup.chmod(0o700)
    saved = {}
    for path in FILES:
        target = LIVE / path
