"""Hash-bound E4 payment-terminal rendering deployment; only the Relay service may be restarted.
...
REPO = Path('/root')
ROOT = REPO / 'output/manual/e4-payment-terminal-20260908'
DOCS = REPO / 'docs/e4-payment-terminal'
LIVE = Path('/opt/obsidian-exchange')
PREIMAGES = Path('/var/lib/obsidian-exchange/deployment-preimages')
RECIPE = 'deploy/e4_payment_terminal_rollout.py'
MAIN = 'relay-fastapi/main.py'
HTML = 'relay/webapp.html'
FILES = (MAIN,)
BASELINE = {item['path']: item['sha256'] for item in [
    {'path': MAIN, 'sha256': '1297db5e84e36ddec0cdf09808c871ce55c28cfd7c4ca04bded9bbace5dde2ab'},
]}
...
def write_report(path, data):
    fd, name = tempfile.mkstemp(prefix='.payment-terminal-report-', dir=path.parent)
    try:
...
def replace(target, expected, content, saved):
    ...
    fd, name = tempfile.mkstemp(prefix='.payment-terminal-', dir=target.parent)
    try:
        with os.fdopen(fd, 'wb') as stream:
    ...
    for path, method, expected in (('/webapp', 'GET', 200),
        ...
        if path == '/webapp' and method == 'GET':
            ...
            record.update(publicMatchesDeployedTemplate=True,
                          publicBody={'path': URL + path, 'sha256': sha(body)})
...
def gates(candidate):
    for name in ('tests.json', 'acceptance-review.json', 'security-review.json',
        ...
        if name == 'payment-browser-report.json':
            ...
            require(report['runnerSha256'] == file_state(REPO / 'tests/e4_payment_terminal_browser.cjs'),
...
def deploy():
    ...
    backup = Path(tempfile.mkdtemp(prefix='e4-payment-terminal-20260908-', dir=PREIMAGES))
    backup.chmod(0o700)
    saved = {}
    for path in FILES:
        target = LIVE / path
