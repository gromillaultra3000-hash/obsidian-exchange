...
_CONTOUR_BY_CURRENCY = {
    ...
    # Monero выдаётся РУКАМИ — решение владельца от 05.08.2026. Авто-выплата
    # потребовала бы monero-wallet-rpc на этой же машине: новый демон, новый
    # ключ и новая поверхность атаки рядом с платёжным контуром. Запись здесь
    # обязательна: без неё монета получила бы 'unknown', а 'unknown' — это «никто
    # не решал», и панель резервов не смогла бы честно сказать, чем мы платим.
    "XMR": "manual",
}
