...
def ton_account_key(address) -> str:
    """Один счёт TON — одна строка, независимо от формы записи.

    Счёт живёт в трёх видах: сырой `0:83df…`, дружественный `UQ…` и bounceable
    `EQ…`. Обозреватель отдаёт сырую форму, кошелёк и настройки владельца — как
    правило дружественную. Сравнение таких строк не «иногда ошибается»: оно не
    совпадает НИКОГДА, и сверка молча перестаёт узнавать собственные переводы.
    Пусто — адрес не разобран (сравнивать нечего, и подставлять исходную строку
    вместо ключа нельзя: тогда мусор совпал бы сам с собой).
    """
    wc, h = parse_ton_address(address)
    return "" if wc is None else f"{wc}:{h.hex()}"
