...
def _fake(url, timeout=12):
    ...
    if "action=txlistinternal" in url:
        ...
        # Первая запись — дубль внешней 0x1 (транзакция двигала ETH и напрямую,
        # и через контракт), в истории она должна остаться ОДНОЙ строкой.
        return {"status": "1", "result": [
            {"hash": "0x1", "from": "0xdead", "to": "0xME",
             "value": "1000000000000000000", "timeStamp": "1700000000", "isError": "0"},
...
# Приход контрактом (вывод с биржи), перевод USDT и обычный перевод ETH — по
# убыванию времени. Сорвавшаяся, нулевой вызов и дубль внутренней отброшены.
check(kinds == [("ETH", "in", 2.0), ("USDT", "in", 25.0), ("ETH", "in", 1.0)],
      f"история собрана неверно (внутренние переводы, порядок по времени, "
      f"срыв, нулевой вызов и дубль): {kinds}")
check(len([i for i in h["items"] if i["txid"] == "0x1"]) == 1,
      "одна транзакция попала в историю дважды — клиент прочитает два перевода")
