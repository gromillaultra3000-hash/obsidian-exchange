...
const origin = 'https://tonconnect-intent.invalid', initial = 'EQ' + 'A'.repeat(46), verified = 'EQ' + 'B'.repeat(46);
...
async function main() {
    ...
    try {
        for (const width of [320, 390]) for (const scenario of ['stale-after-retirement', 'valid', 'unsolicited', 'wrong-nonce', 'duplicate', 'old-during-new-prep', 'old-during-new-intent', 'away-back', 'null-disconnect', 'retry-after-error', 'late-verify-edit']) {
            ...
            await context.route('**/*', async route => {
                ...
                if (u.origin === origin && u.pathname === '/api/tonconnect/verify' && r.method() === 'POST') {
                    ...
                    if (scenario === 'duplicate' || scenario === 'late-verify-edit') {heldVerify = route; return;}
            ...
            if (scenario === 'unsolicited') {
                const before = await state(); await callback('unsolicited');
                assert.deepEqual(await state(), before); assert.equal(verifications.length, 0);
            } else {
                await prepare(); assert.equal(payloads.length, 1);
                const nonce = payloads[0];
                if (scenario === 'stale-after-retirement') await page.locator('#address').fill('EQ' + 'C'.repeat(46));
                if (scenario === 'away-back') {await page.locator('#address').fill('other'); await page.locator('#address').fill(initial);}
                if (scenario === 'null-disconnect') await page.evaluate(() => tcHandleWallet(null));
                if (['stale-after-retirement', 'away-back', 'null-disconnect'].includes(scenario)) {
                    const before = await state(); await callback(nonce); assert.deepEqual(await state(), before); assert.equal(verifications.length, 0);
                } else if (scenario.startsWith('old-during-new-')) {
                    await page.locator('#address').fill('EQ' + 'C'.repeat(46));
                    if (scenario === 'old-during-new-prep') {
                        await page.evaluate(() => {window.__newPreparation = tcConnect();});
                        for (let n = 0; !heldPayload && n < 100; n++) await new Promise(resolve => setTimeout(resolve, 10));
                        assert.ok(heldPayload);
                    } else await prepare();
                    const before = await state(); await callback(nonce); assert.deepEqual(await state(), before); assert.equal(verifications.length, 0);
                    if (heldPayload) {
                        await heldPayload.fulfill({contentType: 'application/json', body: JSON.stringify({payload: payloads[1]})});
                        await page.evaluate(() => window.__newPreparation);
                    }
                    await callback(payloads[1]); assert.equal(verifications.length, 1); assert.equal((await state()).address, verified);
                } else if (scenario === 'wrong-nonce') {
                    const before = await state(); await callback('wrong-nonce'); assert.deepEqual(await state(), before); assert.equal(verifications.length, 0);
                    await callback(nonce); assert.equal(verifications.length, 1); assert.equal((await state()).address, verified);
                } else if (scenario === 'duplicate' || scenario === 'late-verify-edit') {
                    await page.evaluate(payload => {window.__verification = tcHandleWallet({account: {address: 'synthetic'}, connectItems: {tonProof: {proof: {payload, synthetic: true}}}});}, nonce);
                    for (let n = 0; !heldVerify && n < 100; n++) await new Promise(resolve => setTimeout(resolve, 10)); assert.ok(heldVerify);
                    if (scenario === 'duplicate') await callback(nonce);
                    else await page.locator('#address').fill('EQ' + 'C'.repeat(46));
                    ...
                    assert.equal((await state()).address, scenario === 'duplicate' ? verified : 'EQ' + 'C'.repeat(46));
                } else if (scenario === 'retry-after-error') {
                    failVerify = true; await callback(nonce); assert.equal(verifications.length, 1); assert.equal((await state()).address, initial);
                    await callback(nonce); assert.equal(verifications.length, 1);
                    failVerify = false; await prepare(); await callback(payloads[1]); assert.equal(verifications.length, 2); assert.equal((await state()).address, verified);
                } else {await callback(nonce); assert.equal(verifications.length, 1); assert.equal((await state()).address, verified);}
            }
