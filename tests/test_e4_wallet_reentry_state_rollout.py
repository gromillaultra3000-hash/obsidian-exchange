...
import pytest
spec=importlib.util.spec_from_file_location('rollout',str(Path(__file__).resolve().parents[1] / 'docs/e4-wallet-reentry-state/rollout.py'))
m=importlib.util.module_from_spec(spec);spec.loader.exec_module(m)
@pytest.fixture
def env(tmp_path,monkeypatch):
    before=subprocess.check_output(['git','show','31122f2:relay/webapp.html'],cwd='/root')
...
def test_failed_replace_preserves_baseline(env,monkeypatch):
    ...
    assert t.read_bytes()==before
    assert (b/'webapp.html').read_bytes()==before
    assert not list(t.parent.glob('.e4-wallet-reentry-state-*'))
...
@pytest.mark.parametrize('marker', [
    ...
    b'// One unresolved handoff per tab.',
    b'(function wireWalletAttemptNotice() {',
])
def test_missing_addition_rejected(env, marker):
    *_, before, after = env
    with pytest.raises(RuntimeError, match='Addition boundary missing'):
