...
def ops(tmp_path, monkeypatch):
    source = Path(__file__).resolve().parents[1] / 'deploy/e4_payment_status_read_rollout.py'
    spec = importlib.util.spec_from_file_location('e4_payment_status_ops_test', source)
    ...
    monkeypatch.setattr(module, 'safe_runtime_flags', lambda: {'enabled': False, 'present': False})
    calls = []
    ...
    return SimpleNamespace(module=module, calls=calls, snapshots=snapshots,
                           old=old, new=new, real_gates=real_gates)
...
def test_new_module_is_retained_until_old_main_restart_succeeds(ops, monkeypatch):
    ...
    def interrupted_restart(report, state):
        ...
        assert all((m.LIVE / path).read_bytes() == ops.new[path] for path in m.ADDITIVE)
    ...
    with pytest.raises(RuntimeError, match='synthetic restart interruption'):
        m.rollback()
    assert all((m.LIVE / path).exists() for path in m.ADDITIVE)
    assert len(ops.calls) == 1
...
def test_active_autopilot_rejected_without_service_mutation(ops, monkeypatch):
    ...
    spec = importlib.util.spec_from_file_location('e4_payment_status_ops_guard', Path(__file__).resolve().parents[1] / m.RECIPE)
...
def test_enabled_or_unknown_runtime_read_flag_is_never_exposed(ops, monkeypatch):
    m = ops.module
    spec = importlib.util.spec_from_file_location('e4_payment_status_ops_flags', Path(__file__).resolve().parents[1] / m.RECIPE)
    original = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(original)
    monkeypatch.setattr(original, 'unit_state', lambda _: {'MainPID': '123'})
    payload = b''
    monkeypatch.setattr(original, 'Path', lambda *args: SimpleNamespace(read_bytes=lambda: payload))
    assert original.safe_runtime_flags()['present'] is False
    for value in (b'0', b'false', b'no'):
        payload = b'UNRELATED=synthetic\0RELAY_P3_AUTHORIZED_READ_FUNCTIONS_ENABLED=' + value
        assert original.safe_runtime_flags() == {
            'setting': 'RELAY_P3_AUTHORIZED_READ_FUNCTIONS_ENABLED',
            'present': True, 'enabled': False}
    for value in (b'1', b'true', b'yes', b'unrecognized-synthetic-value'):
        payload = b'UNRELATED=synthetic\0RELAY_P3_AUTHORIZED_READ_FUNCTIONS_ENABLED=' + value
        with pytest.raises(RuntimeError, match='runtime_read_rpc_flag_not_disabled') as error:
            original.safe_runtime_flags()
        assert value.decode() not in str(error.value)
    assert not ops.calls


def test_runtime_read_flag_changed_since_preflight_rejects_before_mutation(ops, monkeypatch):
    m = ops.module
    monkeypatch.setattr(m, 'safe_runtime_flags', lambda: {'enabled': False, 'present': True})
    with pytest.raises(RuntimeError, match='runtime_read_flag_changed'):
        m.deploy()
    assert not m.STATE.exists()
    assert not ops.calls
...
def test_live_html_baseline_drift_rejects_before_mutation(ops):
    ...
    with pytest.raises(RuntimeError, match='baseline_drift'):
        m.deploy()
    assert not m.STATE.exists()
    assert not ops.calls


def test_both_additive_modules_are_removed_before_rollback_public_probe(ops, monkeypatch):
    m = ops.module
    m.deploy()
    original_verify = m.verify_runtime

    def verify(report, template):
        assert all(not (m.LIVE / path).exists() for path in m.ADDITIVE)
        assert (m.LIVE / m.MAIN).read_bytes() == ops.old[m.MAIN]
        return original_verify(report, template)
...
def test_all_current_evidence_gates_bind_inputs_and_require_browser_cleanup(ops):
    m = ops.module
    ...
    runner = m.REPO / 'tests/e4_review_browser.cjs'
    runner.parent.mkdir(parents=True, exist_ok=True)
    runner.write_bytes(b'synthetic runner')
    browser = {'result': 'PASS', 'sourceSha256': candidate[m.HTML],
               'runnerSha256': m.sha(runner.read_bytes())}
    (m.DOCS / 'browser-report.json').write_text(json.dumps(browser))
    ...
    (m.DOCS / 'browser-isolation.json').write_text(json.dumps(isolation))
