...
const origin='https://wallet-cross-tab.invalid',wallet='0:'+'b'.repeat(64),address='EQ'+'A'.repeat(46),key='oe.wallet-attempt.shared.v1',lockName='oe.wallet-handoff.v1';
...
const report={sourceSha256:hash(source),runnerSha256:hash(fs.readFileSync(__filename)),baseline,playwrightVersion:require('playwright-core/package.json').version,checks:[],unexpectedWrites:[],pageErrors:[]};
fs.mkdirSync(output,{recursive:true});
async function main(){
 assert.notEqual(process.getuid(),0);const browser=await chromium.launch({executablePath:'/opt/google/chrome/chrome',chromiumSandbox:true,headless:true});report.browserVersion=browser.version();let page;
 try{for(const width of [320,390]){
  async function fixture(modes=['pending','pending']){
   ...
   const pages=[];for(const mode of modes){page=await context.newPage();page.setDefaultTimeout(4000);page.on('pageerror',e=>report.pageErrors.push(e.message));await page.goto(origin+'/webapp',{waitUntil:'load'});await setup(page,mode);pages.push(page);}return {context,pages,counts};
  }
  ...
  async function open(p,kind='transfer'){await p.locator(kind==='transfer'?'#w-send-go':'.wallet-pay').click();await p.locator('#exchange-review').waitFor({state:'visible'});assert.equal(await p.locator('#exchange-review-ack').isChecked(),false);await p.locator('#exchange-review-ack').check();}
  ...
  {
   const f=await fixture(),[a,b]=f.pages;await open(a);await open(b,'payment');
