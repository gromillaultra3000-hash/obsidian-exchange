...
def ops(tmp_path, monkeypatch):
    source = Path(__file__).resolve().parents[1] / 'deploy/e4_payment_terminal_rollout.py'
    spec = importlib.util.spec_from_file_location('e4_payment_terminal_ops_test', source)
...
def test_active_autopilot_rejected_without_service_mutation(ops, monkeypatch):
    ...
    spec = importlib.util.spec_from_file_location('e4_payment_terminal_ops_guard', Path(__file__).resolve().parents[1] / m.RECIPE)
...
def test_evidence_gates_bind_candidate_fixture_runner_and_browser_cleanup(ops):
    ...
    runner = m.REPO / 'tests/e4_payment_terminal_browser.cjs'
