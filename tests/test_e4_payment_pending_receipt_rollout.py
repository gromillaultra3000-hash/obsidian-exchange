...
def ops(tmp_path, monkeypatch):
    source = Path(__file__).resolve().parents[1] / 'deploy/e4_payment_pending_receipt_rollout.py'
    spec = importlib.util.spec_from_file_location('e4_payment_pending_receipt_ops_test', source)
...
def test_active_autopilot_rejected_without_service_mutation(ops, monkeypatch):
    ...
    spec = importlib.util.spec_from_file_location('e4_payment_pending_receipt_ops_guard', Path(__file__).resolve().parents[1] / m.RECIPE)
...
def test_evidence_gates_bind_candidate_fixture_runner_and_browser_cleanup(ops):
    ...
    runner = m.REPO / 'tests/e4_payment_pending_receipt_browser.cjs'
...
@pytest.mark.parametrize('mutation,error', [
    ...
    ('effectful', 'numeric_presentation_effectful_call'),
])
def test_scope_guard_allows_only_bounded_presentation(ops, mutation, error):
    m = ops.module
    old = SCOPE_BASELINE.encode()
    candidate = SCOPE_BASELINE.replace("return 'old'", "return 'new'").replace(
        "_t = 'old title'", "_t = 'new title'")
    if mutation == 'outside':
        ...
        candidate = candidate.replace("_t = 'new title'", '_t = _titles.clear()')
