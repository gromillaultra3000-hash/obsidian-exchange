#!/usr/bin/env python3
"""SELL estimate rollout; reuse reviewed atomic file/metadata/runtime operations."""
...
INVENTORY = ROOT / 'docs/e4-sell-estimate/ops-baseline.json'
spec = importlib.util.spec_from_file_location('e4_reviewed_rollout', HELPER)
shared = importlib.util.module_from_spec(spec)
spec.loader.exec_module(shared)
shared.NAMES = ('loadSellOptions', 'sellCalc', 'createSellOrder')
shared.SCOPE = ['sellOptions state and sellSide entry refresh', 'sellSnapshotInfo and sellReviewEstimate helpers', 'loadSellOptions and sellCalc', 'createSellOrder snapshot disclosure', 'sell-calc-box indicative status and refresh control']
REGIONS = (
    rb'(?m)^        let sellOptions = \{\};\n.*?(?=^        async function loadSellPending\(\))',
    rb'(?m)^                <div class="fee-box" id="sell-calc-box">\n.*?^                </div>\n',
)
...
def scoped(data):
    for index, pattern in enumerate(REGIONS):
        ...
        shared.require(count == 1, 'Region boundary missing/ambiguous')
    for name in ('sellSnapshotInfo', 'sellReviewEstimate'):
