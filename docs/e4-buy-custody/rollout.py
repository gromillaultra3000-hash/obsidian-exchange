#!/usr/bin/env python3
"""BUY disclosure rollout; reuse reviewed atomic file/metadata/runtime operations."""
...
INVENTORY = ROOT / 'docs/e4-buy-custody/ops-baseline.json'
spec = importlib.util.spec_from_file_location('e4_reviewed_rollout', HELPER)
shared = importlib.util.module_from_spec(spec)
spec.loader.exec_module(shared)
shared.NAMES = ('beginBuyOrder',)
shared.SCOPE = ['beginBuyOrder: recipient custody row and irreversible payout risk text']
shared.scoped = shared.stripped
REPLACEMENTS = (
    ("{label: 'Custody', value: 'Ключи и адрес получателя остаются под вашим контролем'}",
     "{label: 'Кто хранит полученные средства', value: 'В личном кошельке ключами управляете вы. Если это адрес биржи или другого сервиса, средства хранит этот сервис; его правила доступа и KYC действуют отдельно. ObsidianExchange не получает ключи кошелька получателя.'}"),
    ("risk: 'После создания появятся реквизиты для оплаты. Не переводите деньги вне заявки и не используйте адрес другой сети.'",
     "risk: 'После создания появятся реквизиты для оплаты. Не переводите деньги вне заявки. Проверьте адрес, сеть и тег / memo получателя: после отправки выплаты в блокчейн отменить перевод нельзя.'"),
)


def bounded(before, after):
    shared.require(before != after, 'Empty candidate')
    shared.require(shared.stripped(before) == shared.stripped(after), 'Changes outside beginBuyOrder')
    expected = before
    for old, new in REPLACEMENTS:
        old, new = old.encode(), new.encode()
        shared.require(expected.count(old) == 1, 'Missing/ambiguous old disclosure')
        expected = expected.replace(old, new)
    shared.require(expected == after, 'Changes outside two exact disclosure replacements')
