...
s = r.shared
baseline = subprocess.check_output(['git', 'show', 'a81ed071fa45341a0b472722bb11fa1b6cd8991f:relay/webapp.html'], cwd=r.ROOT)
...
checks.append('exact_two_disclosure_replacements_accepted')
reject('unrelated_bytes_rejected', lambda: r.bounded(baseline, candidate + b'\n'))
reject('beginBuyOrder_logic_mutation_rejected', lambda: r.bounded(baseline, candidate.replace(b'submitBuyOrder({currency, amount, address, payMethod, destTag, noTag, network})', b'submitBuyOrder({currency, amount, address})')))
reject('only_one_disclosure_replacement_rejected', lambda: r.bounded(baseline, baseline.replace(r.REPLACEMENTS[0][0].encode(), r.REPLACEMENTS[0][1].encode())))
reject('empty_candidate_rejected', lambda: r.bounded(baseline, baseline))
with tempfile.TemporaryDirectory(prefix='e4-buy-custody-rehearsal-') as directory:
    root = Path(directory)
