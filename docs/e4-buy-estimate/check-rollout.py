...
checks.append('bounded_estimate_scope_accepted')
...
reject('calculateFee_mutation_rejected', lambda: r.bounded(baseline, candidate.replace(b'function calculateFee()', b'function changedCalculateFee()')))
reject('empty_candidate_rejected', lambda: r.bounded(baseline, baseline))
with tempfile.TemporaryDirectory(prefix='e4-buy-estimate-rehearsal-') as directory:
    root = Path(directory)
