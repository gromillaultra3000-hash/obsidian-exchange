#!/usr/bin/env python3
"""TON Connect recipient binding rollout; reuse reviewed atomic file/metadata/runtime operations."""
...
INVENTORY = ROOT / 'docs/e4-tonconnect-binding/ops-baseline.json'
spec = importlib.util.spec_from_file_location('e4_reviewed_rollout', HELPER)
shared = importlib.util.module_from_spec(spec)
spec.loader.exec_module(shared)
shared.NAMES = ('tcHandleWallet',)
shared.SCOPE = ['tcRecipientGeneration/helpers/input-change listener block after tcPending', 'tcHandleWallet recipient binding and strict response gate', 'exact invalidation call in applyOfferings retired route', 'exact invalidation call in addressbook chip handler', 'exact invalidation call in updateAddressPlaceholder']
CALL_SITES = (
    (b'                if (previous) {\n                    tcInvalidateRecipient();\n', b'                if (previous) {\n'),
    (b'                b.onclick = () => {\n                    tcInvalidateRecipient();\n', b'                b.onclick = () => {\n'),
    (b'        function updateAddressPlaceholder() {\n            tcInvalidateRecipient();\n', b'        function updateAddressPlaceholder() {\n'),
)


def scoped(data):
    pattern = rb'(?m)^        let tcPending = false;\n.*?(?=^        function tcAvailable\(\))'
    data, count = re.subn(pattern, b'TC_RECIPIENT_STATE_BLOCK\n', data, flags=re.S)
    shared.require(count == 1, 'TC state block boundary missing/ambiguous')
    for new, old in CALL_SITES:
        shared.require(data.count(new) <= 1, 'Invalidation site ambiguous')
...
def bounded(before, after):
    ...
    shared.require(scoped(before) == scoped(after), 'Changes outside authorized TON recipient binding scope')
    for new, old in CALL_SITES:
        shared.require(before.count(new) == 0 and after.count(new) == 1, 'Missing/unexpected exact invalidation addition')
