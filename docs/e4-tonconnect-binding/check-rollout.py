...
checks.append('bounded_ton_recipient_binding_scope_accepted')
...
reject('missing_addressbook_invalidation_rejected', lambda: r.bounded(baseline, candidate.replace(r.CALL_SITES[1][0], r.CALL_SITES[1][1])))
...
reject('wallet_connect_payload_mutation_rejected', lambda: r.bounded(baseline, candidate.replace(b'async function tcConnect()', b'async function changedTcConnect()')))
reject('empty_candidate_rejected', lambda: r.bounded(baseline, baseline))
with tempfile.TemporaryDirectory(prefix='e4-tonconnect-binding-rehearsal-') as directory:
    root = Path(directory)
