...
NAMES = ('submitBuyOrder', 'submitSellOrder')
...
def atomic(path, data, meta):
    fd, temp = tempfile.mkstemp(prefix='.e4-money-flow-', dir=path.parent)
    try:
...
def prepare(args):
    baseline_path = ROOT / 'docs/e4-money-flow-coverage/ops-baseline.json'
