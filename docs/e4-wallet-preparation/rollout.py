...
NAMES = ('walletTransfer', 'walletPay', 'closeExchangeReview', 'openExchangeReview', 'invalidateExchangeReview')
...
def stripped(data):
    data = data.replace(b'        // Pending wallet responses belong to one review attempt only.\n        let exchangeReviewPreparationGeneration = 0;\n', b'')
    for name in NAMES:
...
def atomic(path, data, meta):
    fd, temp = tempfile.mkstemp(prefix='.e4-wallet-preparation-', dir=path.parent)
    try:
...
def prepare(args):
    baseline_path = ROOT / 'docs/e4-wallet-preparation/ops-baseline.json'
