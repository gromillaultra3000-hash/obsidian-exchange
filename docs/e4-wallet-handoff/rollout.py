...
def stripped(data):
    data = data.replace(b'        let walletHandoffPending = false;\n', b'')
    for name in NAMES:
...
def atomic(path, data, meta):
    fd, temp = tempfile.mkstemp(prefix='.e4-wallet-handoff-', dir=path.parent)
    try:
...
def prepare(args):
    baseline_path = ROOT / 'docs/e4-wallet-handoff/ops-baseline.json'
