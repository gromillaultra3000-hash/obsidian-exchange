...
with tempfile.TemporaryDirectory(prefix='e4-tonconnect-preparation-harness-') as temporary:
    root = Path(temporary)
    (root / 'tests').mkdir()
    shutil.copyfile(ROOT / 'tests/e4_tonconnect_preparation_browser.cjs', root / 'tests/e4_review_browser.cjs')
