...
checks.append('bounded_ton_preparation_scope_accepted')
...
reject('verify_response_logic_mutation_rejected', lambda: r.bounded(baseline, candidate.replace(b'data.verified === true', b'data.verified == true')))
...
with tempfile.TemporaryDirectory(prefix='e4-tonconnect-preparation-rehearsal-') as directory:
    root = Path(directory)
