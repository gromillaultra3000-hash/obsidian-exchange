#!/usr/bin/env python3
"""TON Connect preparation binding rollout; reuse reviewed atomic file/metadata/runtime operations."""
...
INVENTORY = ROOT / 'docs/e4-tonconnect-preparation/ops-baseline.json'
spec = importlib.util.spec_from_file_location('e4_reviewed_rollout', HELPER)
shared = importlib.util.module_from_spec(spec)
spec.loader.exec_module(shared)
shared.NAMES = ('tcConnect',)
shared.SCOPE = ['tcPreparation declaration after tcRecipientGeneration', 'tcHandleWallet expected-disconnect preparation preamble', 'tcConnect serialized route-bound payload preparation']


def scoped(data):
    old = b'        let tcRecipientGeneration = 0;\n'
    new = old + b'        let tcPreparation = null;\n'
    shared.require(data.count(new) <= 1, 'Preparation state ambiguous')
    data = data.replace(new, old)
    pattern = rb'(?m)^        async function tcHandleWallet\(w\) \{\n.*?(?=^            const recipientState = tcRecipientState\(\);)'
    data, count = re.subn(pattern, b'TC_EXPECTED_DISCONNECT_PREAMBLE\n', data, flags=re.S)
...
def bounded(before, after):
    ...
    shared.require(scoped(before) == scoped(after), 'Changes outside authorized TON preparation scope')
    shared.require(before.count(b'        let tcPreparation = null;') == 0
        and after.count(b'        let tcPreparation = null;') == 1, 'Missing/unexpected preparation state addition')
