...
ROOT=Path('/root')
RECIPE=ROOT/'deploy/e4_payment_terminal_rollout.py'
MAIN=ROOT/'relay-fastapi/main.py'
OLD_SOURCE=subprocess.run(['git','show','4c7882c15d6d88ff80d56d2cc160076eff929f95:relay-fastapi/main.py'],cwd=ROOT,capture_output=True,check=True).stdout
cases=[]
...
for scenario in ['normal-round-trip','before-publication','after-publication','restart-acknowledgement-lost']:
    with tempfile.TemporaryDirectory(prefix='terminal-security-ops-') as temporary:
...
print(json.dumps({'schemaVersion':'e4-payment-terminal-independent-ops-probe.v1','result':'PASS',
