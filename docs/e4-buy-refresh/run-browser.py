...
with tempfile.TemporaryDirectory(prefix='e4-buy-refresh-harness-') as temporary:
    root = Path(temporary)
    (root / 'tests').mkdir()
    shutil.copyfile(ROOT / 'tests/e4_buy_refresh_browser.cjs', root / 'tests/e4_review_browser.cjs')
