#!/usr/bin/env python3
"""Temporary-filesystem rehearsal of actual three-file operations and drift guards."""
import copy
...
with tempfile.TemporaryDirectory(prefix='e4-site-rollout-') as directory:
    ...
    checks.extend(['three_file_apply_metadata_pass', 'reconcile_pass'])
    ...
    assert not (live / r.NAMES[0]).exists()
    checks.append('exact_rollback_removes_new_include_preserves_metadata')
    atomic = s.atomic
