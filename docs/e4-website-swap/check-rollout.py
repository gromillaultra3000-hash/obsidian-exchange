#!/usr/bin/env python3
"""Temporary-filesystem rehearsal of four-file operations and relay-only restart."""
import copy
...
with tempfile.TemporaryDirectory(prefix='e4-site-rollout-') as directory:
    ...
    for i, entry in enumerate(fixture['targets']):
        ...
        before = None if i < 2 else Path(entry['target']).read_bytes()
    ...
    def fake_restart(command, **kwargs):
        ...
        if not restarts:
            assert r.digest(Path(fixture['targets'][2]['target'])) == fixture['targets'][2]['baseline_sha']
            assert r.digest(Path(fixture['targets'][3]['target'])) == fixture['targets'][3]['candidate_sha']
    ...
    unrelated.write_bytes(b'unrelated')
    target = Path(fixture['targets'][2]['target']); target.chmod(0o600)
    ...
    checks.extend(['four_file_apply_metadata_pass', 'reconcile_pass_without_restart', 'only_relay_restarted_once', 'backend_activated_before_quote_only_form'])
    ...
    assert all(not (live / Path(n).name).exists() for n in r.NAMES[:2])
    assert len(restarts) == 2
    checks.append('failed_relay_rollback_removes_new_helper_and_include_preserves_metadata_restarts_relay')
    fixture['units'][0]['state'] = real_state[r.UNIT].copy()
    atomic = s.atomic
...
result = dict(result='PASS', checks=checks, production_mutated=False,
              limitations='Temporary filesystem; fixture service identities and public responses. Real plan binding checked separately; public auth redirects checked in prepare.')
