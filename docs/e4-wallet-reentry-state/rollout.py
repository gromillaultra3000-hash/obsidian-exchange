...
ADDITIONS = (
    rb'(?m)^        <section id="wallet-attempt-notice"[^\n]*\n.*?^        </section>\n\n',
    rb'(?m)^        // One unresolved handoff per tab\..*?^        let exchangeReviewCommit',
    rb'(?m)^        \(function wireWalletAttemptNotice\(\) \{\n.*?^        \}\)\(\);\n\n',
)
SCOPE = list(NAMES) + ['wallet-attempt-notice', 'walletAttempt helpers', 'wireWalletAttemptNotice']
...
def bounded(before, after):
    ...
    candidate = after
    for index, pattern in enumerate(ADDITIONS):
        require(not re.search(pattern, before, flags=re.S), 'Addition already present in baseline')
        replacement = b'        let exchangeReviewCommit' if index == 1 else b''
        candidate, count = re.subn(pattern, replacement, candidate, flags=re.S)
        require(count == 1, 'Addition boundary missing/ambiguous: ' + str(index))
    require(stripped(before) == stripped(candidate), 'Changes outside authorized scope')
...
def atomic(path, data, meta):
    fd, temp = tempfile.mkstemp(prefix='.e4-wallet-reentry-state-', dir=path.parent)
    try:
...
def prepare(args):
    baseline_path = ROOT / 'docs/e4-wallet-reentry-state/ops-baseline.json'
    ...
    plan = dict(target=TARGET, candidate=str(args.candidate.resolve()), candidate_sha=sha(candidate),
                ...
                units=baseline['units'], scope=SCOPE)
...
def execute(plan, action, backup):
    ...
    runtime(plan)
    require(plan['scope'] == SCOPE, 'Invalid rollout scope')
