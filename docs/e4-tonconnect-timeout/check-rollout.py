...
checks.append('bounded_ton_timeout_scope_accepted')
...
reject('connection_preparation_mutation_rejected', lambda: r.bounded(baseline, candidate.replace(b'async function tcConnect()', b'async function changedTcConnect()')))
...
with tempfile.TemporaryDirectory(prefix='e4-tonconnect-timeout-rehearsal-') as directory:
    root = Path(directory)
