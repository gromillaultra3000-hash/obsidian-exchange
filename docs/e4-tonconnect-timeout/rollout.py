#!/usr/bin/env python3
"""TON Connect verification timeout rollout; reuse reviewed atomic file/metadata/runtime operations."""
...
INVENTORY = ROOT / 'docs/e4-tonconnect-timeout/ops-baseline.json'
spec = importlib.util.spec_from_file_location('e4_reviewed_rollout', HELPER)
shared = importlib.util.module_from_spec(spec)
spec.loader.exec_module(shared)
shared.NAMES = ('tcHandleWallet',)
shared.SCOPE = ['tcHandleWallet finite response/body verification wait and timeout feedback']


def bounded(before, after):
    shared.require(before != after, 'Empty candidate')
    shared.require(shared.stripped(before) == shared.stripped(after), 'Changes outside authorized TON verification timeout scope')
