...
with tempfile.TemporaryDirectory(prefix='e4-tonconnect-sdk-wait-harness-') as temporary:
    root = Path(temporary)
    (root / 'tests').mkdir()
    shutil.copyfile(ROOT / 'tests/e4_tonconnect_sdk_wait_browser.cjs', root / 'tests/e4_review_browser.cjs')
