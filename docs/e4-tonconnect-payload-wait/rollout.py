#!/usr/bin/env python3
"""TON Connect payload wait recovery rollout; reuse reviewed atomic file/metadata/runtime operations."""
...
INVENTORY = ROOT / 'docs/e4-tonconnect-payload-wait/ops-baseline.json'
spec = importlib.util.spec_from_file_location('e4_reviewed_rollout', HELPER)
shared = importlib.util.module_from_spec(spec)
spec.loader.exec_module(shared)
shared.NAMES = ('tcConnect',)
shared.SCOPE = ['tcConnect finite payload response/body wait and timeout recovery']


def bounded(before, after):
    shared.require(before != after, 'Empty candidate')
    shared.require(shared.stripped(before) == shared.stripped(after), 'Changes outside authorized TON payload wait scope')
