...
REGIONS = (
    ...
    rb'(?m)^        // One unresolved handoff (?:per tab|across cooperating same-origin tabs)\..*?^        let exchangeReviewCommit',
    rb'(?m)^        \(function wireExchangeReview\(\) \{\n.*?^        \}\)\(\);\n',
    rb'(?m)^        \(function wireWalletAttemptNotice\(\) \{\n.*?^        \}\)\(\);\n\n',
)
SCOPE = list(NAMES) + ['wallet-attempt-notice', 'walletAttempt helpers', 'wireWalletAttemptNotice', 'wireExchangeReview grant capture']
...
def scoped(data):
    for index, pattern in enumerate(REGIONS):
        data, count = re.subn(pattern, b'REGION:' + str(index).encode() + b'\n', data, flags=re.S)
        require(count == 1, 'Region boundary missing/ambiguous: ' + str(index))
    return stripped(data)


def bounded(before, after):
    require(before != after, 'Empty candidate')
    require(scoped(before) == scoped(after), 'Changes outside authorized scope')
...
def atomic(path, data, meta):
    fd, temp = tempfile.mkstemp(prefix='.e4-wallet-cross-tab-', dir=path.parent)
    try:
...
def prepare(args):
    baseline_path = ROOT / 'docs/e4-wallet-cross-tab/ops-baseline.json'
