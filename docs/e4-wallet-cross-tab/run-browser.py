...
with tempfile.TemporaryDirectory(prefix='e4-wallet-cross-tab-harness-') as temporary:
    root = Path(temporary)
    (root / 'tests').mkdir()
    shutil.copyfile(ROOT / 'tests/e4_wallet_cross_tab_browser.cjs', root / 'tests/e4_review_browser.cjs')
