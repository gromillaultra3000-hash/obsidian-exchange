...
ROOT = Path('/root')
TEST = ROOT / 'tests/test_e4_activity_session_rollout.py'
RECIPE = ROOT / 'deploy/e4_activity_session_rollout.py'
...
print(json.dumps({'schemaVersion': 'e4-activity-session-independent-ops-probe.v1',
    'result': 'PASS', 'inputs': [{'path': str(p.relative_to(ROOT)),
    'sha256': hashlib.sha256(p.read_bytes()).hexdigest()} for p in [TEST, RECIPE]],
