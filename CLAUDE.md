...
| VertuProvider | ❌ auth по VERTU_API_KEY работает (balance 0.0), сделки НЕ выдаются. 11.07: найдена и исправлена регрессия — коммит 7743de9 слал `platform_id` вместо `deal_id`, живой API отвечал HTTP 422 «deal_id: Field required» (ломало ВСЕ сделки с 10.07 ещё до подбора трейдера); фикс 59e8a8c вернул `deal_id`. Остаточная ошибка — HTTP 400 «Не удалось выдать сделку» (catch-all, разбор 08.07: суммы 100–100 000 ₽ все 400, фейковый type_pay даёт ту же ошибку → провайдерская сторона: нет трейдеров/баланс 0/мерчант ограничен). Кодом не лечится — писать в поддержку Vertu | 30% |
