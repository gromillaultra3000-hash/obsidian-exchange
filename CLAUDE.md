...
| VertuProvider | ⚠️ ИНТЕРМИТТЕНТНО РАБОТАЕТ (уточнено 11.07 живой диагностикой). Код 100% корректен: после фикса deal_id (59e8a8c) сделка создаётся С ПОЛНЫМИ реквизитами в ОТВЕТЕ create (phone+bank+full_name, status Pending) — поллинг для реквизитов не нужен. sbp 4000/5000 ₽ выдали живые реквизиты (ВТБ, тел.), sbp 6000+/10000/c2c — 400 «Не удалось выдать сделку». Это НЕ порог суммы, а доступность трейдера под конкретную сумму в моменте: 400 = нет свободного трейдера (провайдерская сторона), меняется поминутно. balance 0.0 pay-in НЕ блокирует. Когда трейдер есть — Vertu полностью функционален. Регрессия 422 (7743de9 слал platform_id вместо deal_id) исправлена 59e8a8c. Кодом не лечится, при массовых 400 — писать в поддержку Vertu (мало трейдеров) | 30% |
