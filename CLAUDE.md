...
| LavaProvider | ❌ СНЯТ 04.08.2026 (решение владельца — ключи не заводим). Код остаётся, `LAVA_*` пусты → роутер скипает. Не предлагать как задачу | — |
...
6. PayOK (изучить). ❌ Lava снята с очереди 04.08.2026 — владелец решил ключи не
   заводить. Код провайдера остаётся в репозитории, но роутер его скипает
   (пустой `required_env`), задачей больше не считать.
7. От юзера: SELL_USDT_ADDRESS (USDT-продажа скрыта),
...
### Сессия 27.07.2026 (тревога тонула в своих дублях + ручная выдача мимо котировки)
Сторож конверсии нашёл **3 оплаченные заявки без выдачи крипты — 13 947 ₽
клиентских денег, 19-25 часов** (#99955118 4447₽ BTC, #99955120 6500₽ BTC,
#99955141 3000₽ USDT; оплата подтверждена живьём у Montera и Vertu). Сигнал об
этом уходил админу, но не читался — и вот почему:
- **fix(алерты) 64cada8**: троттлинг «раз в 6 часов» жил в словаре внутри
  корутины, а `deploy.sh` перезапускал relay-fastapi БЕЗУСЛОВНО каждые 15 минут
  (96 рестартов/сутки) → словарь обнулялся вместе с процессом → ~80 одинаковых
  сообщений за сутки. `relay/core/alert_throttle.py` — состояние в БД, захват
  атомарный, fail-OPEN при сбое БД. Ключ = ОТПЕЧАТОК алерта (тип + состав
  заявок), поэтому новая беда пробивает окно молчания сразу. `deploy.sh` —
  гейт по `/root/.deploy_state`, выборочный рестарт, диапазон изменений
  раскатанный→новый вместо `HEAD~1` (один pull = несколько коммитов, правки
  бота терялись). Подробности — в памяти `project-deploy-and-alerting`.
- **fix(деньги) 95ccee4**: панель `/worker` считала объём крипты по СВЕЖЕМУ
  курсу мимо зафиксированной котировки. Страж выплат строгий → к работнику
  уходит большинство заявок, то есть обещание «курс 15 минут» не выполнялось
  на самом частом пути. `payout_verdict()` — одно решение об объёме на все пути
  выдачи (список /worker, карточка отправки, уведомление воркеру).
- Три новые мины в `tests/test_landmines.py` (троттлинг в памяти, безусловный
  рестарт, выдача по курсу мимо котировки) — все проверены на прежнем коде,
  ловят. `tests/test_alert_throttle.py` (17 проверок) + CI. Итого 14/14 наборов.
⚠️ Зависшие заявки авто-выплатой НЕ закроются: на них стоит маркер
`payout_triggered` (защита от двойной отправки) и они старше 24ч. Только
вручную через `/worker` или `/force_payout ID TXID`.
⚠️ **Выплата «мимо бота» оставляет заявку в статусе `paid` навсегда**: клиенту не
уходит уведомление с TXID, реф-бонус и VIP-объём не начисляются, сторож вечно
считает заявку зависшей. Юзер так выдал 118 и 141 — TXID нашлись на цепи по
адресу назначения (BTC `c4fdd58d…a60c43` 0.000642, TRC-20 `4bc19494…caea0c`
28.078 USDT), закрывать через `/force_payout ID TXID`.
- **feat(выплаты) 392b570 — `/payout ORDER_ID`**: запустить движок выплаты руками.
  Раньше было нечем: `/force_payout` только помечает по УЖЕ отправленному TXID, а
  движок жил в фоновом цикле и брал лишь ≤ AUTO_PAYOUT_LIMIT (`/approve` мёртв —
  `pending_large_payouts` никто не наполняет). Команда живёт в боте потому, что
  ключ вольта расшифровывается только в его процессе. Двухшагово: предпросмотр →
  кнопка; все проверки авто-пути (payout_guard, контрольная сумма адреса,
  core.quote, payout_circuit) повторяются ПЕРЕД отправкой. Двойная отправка —
  журнал идемпотентности вольта. ✅ Проверено живьём на #99955120: 0.0009583 BTC,
  tx `2adecb4c…4490`, комиссия 564 сат.
- **feat(сверка) 1e5c1f9 — `relay/core/payout_discovery.py`**: ищет выплаты,
  сделанные мимо системы, по ВХОДЯЩИМ на адрес клиента (chain_reconcile смотрит
  наоборот — исходящие из нашего кошелька и только TRON). BTC/LTC/USDT-TRC20.
  Закрывает сама ТОЛЬКО по переводу с нашего кошелька: совпадение суммы на адресе
  клиента подделывается им самим. `/discover`, `/paysrc add|del CUR АДРЕС`,
  фоновой проход раз в 30 мин. ⚠️ Живой прогон вскрыл тихий дефект первой версии:
  выплата ушла с адреса, которого нет ни в primaryAddress, ни в мете вольта
  (11 против 23 фактических) — список берётся у самого кошелька, кеш на час.
  Детали и грабли — в памяти `project-deploy-and-alerting`.
- **feat(сверка) 2-й заход**: владелец платит из разовых кошельков и в доверенные
  их вносить не будет → ветка «отправитель не наш, решает человек» стала ОСНОВНЫМ
  путём. Спорная находка закрывается КНОПКОЙ прямо из отчёта (`/discover` и фоновое
  уведомление), без копирования TXID в `/force_payout`. Данным из кнопки не
  доверяем: `candidates_for()` пересчитывает всё в момент нажатия. Правило не
  ослаблено — авто-закрытие по-прежнему только по переводу с нашего кошелька.

### Ревизия 27.07.2026 (сплошной аудит сервера за две недели)
Найдено и исправлено (подробности — в памяти `project-server-infra-audit`):
- 🔴 **Инфраструктура спорила сама с собой**: `exchange-relay` (легаси
  relay/app.py, :5000) выключили 19.07 как убийцу платёжных сессий, а
  `watchdog_relay.sh` по крону 21.07 поднял его обратно — работал 6 дней.
  `disable` про автозапуск, не про `restart`. Юнит убран из systemd, сторож
  переписан на relay-fastapi (:5001) с проверкой по эндпоинту.
- 🔴 **Весь shell-мониторинг молчал с 08.07**: `grep ADMIN_ID` ловил и
  `ADMIN_ID_2` → многострочный chat_id → Telegram 400 → curl в /dev/null.
  19 дней тревога «бот упал» не доходила. `/root/lib_tg.sh` — точный разбор,
  доставка всем админам, неудача пишется в watchdog.log.
- 🟠 **`/approve` — мёртвая команда**: `pending_large_payouts` никто не
  наполнял, ответ всегда «нет ожидающей выплаты». Выглядела как 2FA-страховка
  для крупных сумм, которой нет. Убрана, отсылает к `/payout`.
- 🟠 **Подписи тарифов расходились с расчётом на границах**: «до 5 000 ₽», а на
  5 000 уже следующая ступень (границы полуоткрытые). В пользу клиента, потому
  жалоб не было. Теперь «менее 5 000 ₽», «5 000 – 9 999 ₽», …
- Крон: `healthcheck.sh` стоял дважды; `monitor.sh`/`daily_report.sh` дублировали
  питоновский отчёт и не доходили (у monitor.sh ещё и мёртвый источник баланса
  `/root/payout/nowpay.py`) — убраны; токен новостного бота из открытого текста
  в `/root/.news_bot.env` (600). ✅ Закрыто 04.08.2026: новостной бот владельцу
  больше не нужен и убран, светившийся в crontab токен вместе с ним.
- `certbot` вечно failed из-за `pay.obsidianbtc.org` (домен смотрит на ДРУГОЙ
  сервер) — маскировал бы настоящий сбой живых сертификатов. Обновление для него
  отключено обратимо. Живые сертификаты до 23.10.2026.
- `relay/logs/*.log` не ротировались → `/etc/logrotate.d/obsidian` (copytruncate
  обязателен — сервис держит файл открытым).
- Мины: +2 (мёртвое состояние, из которого только читают; выдача по курсу мимо
  котировки). Тестов 15 наборов.
- Проверено и признано НЕ проблемой: права 644 внутри `/root` (каталог 700,
  единственный shell-пользователь — root); бэкапы (sqlite3 .backup + integrity
  check, WAL-безопасно); целостность БД; nginx.
...
- **security**: `/admin/analytics/data` не проверял авторизацию (отдавал аналитику
  ...
  ✅ Токен ротирован владельцем 04.08.2026 (push после ротации проверен живьём).
