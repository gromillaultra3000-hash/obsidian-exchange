...
| VertuProvider | ❌ auth по VERTU_API_KEY работает (balance 0.0), но сделки НЕ выдаются. Разбор 08.07 ночью: суммы 100–100 000 ₽ — все 400; заведомо фейковый type_pay даёт ТУ ЖЕ ошибку «Не удалось выдать сделку» → это catch-all, API не различает «нет трейдеров»/«неверный метод»/«мерчант ограничен». Payload сверен с OpenAPI (amount/deal_id/type_pay — всё корректно). Кодом не лечится — писать в поддержку Vertu | 30% |
