...
# Запускаем polling при старте
@app.on_event("startup")
async def startup():
    logger.info("FastAPI relay started")
    start_monitoring()
...
async def pay_with_token(token: str, request: Request):
    ...
    # Если token – число, обрабатываем как старый order_id
    if token.isdigit():
...
<head>
    ...
    <style>
        ...
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .container {{ width: 100%; max-width: 420px; background: rgba(10,10,15,.88); backdrop-filter: blur(24px); border: 1px solid var(--border); border-radius: 34px; padding: 30px 20px; box-shadow: 0 0 80px rgba(168,85,247,.18), inset 0 0 0 1px rgba(255,255,255,.03); text-align: center; animation: containerFloat 6s ease-in-out infinite; }}
...
# --- Старый формат /pay/{order_id} (числовой ID) ---
...
# --- Gateway Endpoint (скрытый редирект) ---
